...
//kuvan tooted jarjekorraga
for (let i = 0; i < products.length; i++) {
    console.log((i + 1) + ". " + products[i]);
}
console.log("-----------------------------------------------------------")
console.log("Taiendus")
//ei taju hetkel
...
const temperatures = [
    ...
    for (let kuu = 0; kuu < temperatures.length; kuu++) {
        let kuuTemperatuurid = temperatures[kuu];
        let summa = 0;
      
        // Tempside sumpar
        for (let i = 0; i < kuuTemperatuurid.length; i++) {
          summa += kuuTemperatuurid[i];
        }
      
        // Temparite keskar
        let keskmine = summa / kuuTemperatuurid.length;
        // Te
        console.log(`Kuu ${kuu + 1} keskmine temperatuur: ${keskmine.toFixed(2)}`);
      }

      let maxTemperatuur = -Infinity;
let minTemperatuur = Infinity;
let maxKuu;
let minKuu;

// Käige läbi igakuiselt ja leidke maksimaalne ja minimaalne temperatuur
for (let kuu = 0; kuu < temperatures.length; kuu++) {
  let kuuTemperatuurid = temperatures[kuu];
  let kuuNimi = months.split(", ")[kuu];

  // Leia maksimaalne ja minimaalne temperatuur
  let maksimum = Math.max(...kuuTemperatuurid);
  let miinimum = Math.min(...kuuTemperatuurid);

  // Võrdle ja uuenda maksimaalse ja minimaalse temperatuuri väärtusi
  if (maksimum > maxTemperatuur) {
    maxTemperatuur = maksimum;
    maxKuu = kuuNimi;
  }

  if (miinimum < minTemperatuur) {
    minTemperatuur = miinimum;
    minKuu = kuuNimi;
  }
}

console.log(`Kõrgeim temperatuur oli kuus "${maxKuu}" ja see oli ${maxTemperatuur} kraadi.`);
console.log(`Madalaim temperatuur oli kuus "${minKuu}" ja see oli ${minTemperatuur} kraadi.`);



//vannun ema nimel, et sei ole chatgpt kätetöö